...
export async function initSummarizer() {
  try {
    console.log('TabTree: Checking for Chrome AI...');
    console.log('TabTree: window.ai exists?', !!window.ai);

    if (!window.ai) {
      errorMessage = 'Chrome AI not available. Enable at chrome://flags/#optimization-guide-on-device-model';
      console.log('TabTree:', errorMessage);
      console.log('TabTree: Note - Built-in AI is experimental and may require Chrome Canary with flags enabled');
      return false;
    }

    console.log('TabTree: window.ai.summarizer exists?', !!window.ai.summarizer);
    ...
    console.log('TabTree: Checking summarizer capabilities...');
    const capabilities = await window.ai.summarizer.capabilities();
    console.log('TabTree: Capabilities:', capabilities);
    ...
    if (capabilities.available === 'after-download') {
      console.log('TabTree: Summarizer model downloading in background...');
      errorMessage = 'AI model downloading...';
    }

    console.log('TabTree: Creating summarizer...');
    ...
    isAvailable = true;
    errorMessage = null;
    console.log('TabTree: AI summarization ready!');
    return true;
  } catch (error) {
    errorMessage = `Chrome AI error: ${error.message}`;
    console.error('TabTree: Chrome AI initialization failed:', error);
    console.log('TabTree: Stack trace:', error.stack);
    return false;
  }
}
...
export async function generateSummary(pages) {
  ...
  try {
    ...
    console.error('TabTree: Summarization failed:', error);
    return null;
  }
}
